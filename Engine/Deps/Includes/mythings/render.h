...
void renderQuad();
void renderWall();
void renderSphere(int sphereSegments = 64);
...
//绘制屏幕平面(没有设置法向量，不可以当作物体平面绘制，不然会有光照问题)
GLuint quadVAO = 0;
GLuint quadVBO = 0;
void renderQuad()
{
...
void renderSphere(int sphereSegments)
{
	if (sphereVAO == 0)
	{
		...
		const unsigned int X_SEGMENTS = sphereSegments;
		const unsigned int Y_SEGMENTS = sphereSegments;
